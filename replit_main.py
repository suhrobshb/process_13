...
# The new, comprehensive UI template
INDEX_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function startRecording() {
                try {
                    ...
                    showToast('error', 'Error', 'Could not start recording: ' + error.message);
                }
            }
            ...
            async function stopRecording() {
                ...
                try {
                    ...
                    showToast('info', 'Recording Stopped', `Recorded ${recordedActions.length} actions`);
                    updateStatus('Recording stopped. Processing actions...');
                    
                    updateRecordedActionsList();
                    processRecordedActions();
                } catch (error) {
                    showToast('error', 'Error', 'Could not stop recording: ' + error.message);
                }
            }
            ...
            function updateRecordedActionsList() {
                ...
                recordedActions.forEach((action, index) => {
                    const time = new Date(action.timestamp).toTimeString().split(' ')[0];
                    let icon = '';
                    let title = '';
                    let description = '';
            ...
            function createWorkflowFromActions() {
                ...
                recordedActions.forEach((action, index) => {
                    ...
                    switch (action.type) {
                        case 'click':
                            type = 'desktop';
                            params = { action: 'click', x: action.details.x, y: action.details.y };
                            break;
                        case 'type':
                            type = 'desktop';
                            params = { action: 'type', text: action.details.text };
                            break;
                        case 'keypress':
                            type = 'desktop';
                            params = { action: 'press', key: action.details.key };
                            break;
                        case 'scroll':
                            type = 'desktop';
                            params = { action: 'scroll', direction: action.details.direction, amount: action.details.amount };
                            break;
                        case 'navigate':
                            type = 'browser';
                            params = { action: 'goto', url: action.details.url };
                            break;
                    }
                    ...
                    if (type) {
                        ...
                        lastNodeId = node.id;
                        y += 120; // Stagger nodes vertically
                    }
                });
                
                render();
                showToast('success', 'Workflow Created', 'Workflow generated from recorded actions');
                updateStatus('Workflow created from recording');
            }

            // Other functions (drag/drop, rendering, etc.) go here...
            // These would be extensive and are omitted for brevity, but the
            // structure is in place to build them out.
            
            function handleDragStart(e) { /* ... */ }
            function handleDragOver(e) { /* ... */ }
            function handleDrop(e) { /* ... */ }
            function handleRecordedActionDragStart(e) { /* ... */ }
            ...
            function closeSaveWorkflowModal() { /* ... */ }
            async function saveWorkflow() { /* ... */ }
            async function openLoadWorkflowModal() { /* ... */ }
            function closeLoadWorkflowModal() { /* ... */ }
            async function loadWorkflow() { /* ... */ }
            function showSelectedWorkflowDetails() { /* ... */ }
...
async def index(request: Request):
    """Render the main workflow recorder and editor UI."""
    return templates.TemplateResponse("index.html", {"request": request})

@app.post("/recorder/start")
async def start_recording():
    """Mock endpoint to start recording."""
    logger.info("Received request to start recording")
    # In a real application, this would trigger the agent
    return JSONResponse(content={"status": "recording_started"})

@app.post("/recorder/stop")
async def stop_recording():
    """Mock endpoint to stop recording and return simulated actions."""
    logger.info("Received request to stop recording")
    # Simulate a few recorded actions
    actions = [
        {"type": "click", "timestamp": datetime.utcnow().isoformat(), "details": {"x": 100, "y": 200, "element": "button"}},
        {"type": "type", "timestamp": datetime.utcnow().isoformat(), "details": {"text": "Hello, AI Engine!", "element": "input"}},
        {"type": "navigate", "timestamp": datetime.utcnow().isoformat(), "details": {"url": "https://example.com"}},
        {"type": "scroll", "timestamp": datetime.utcnow().isoformat(), "details": {"direction": "down", "amount": 300}},
    ]
    return JSONResponse(content={"status": "recording_stopped", "actions": actions})

@app.post("/api/workflows")
async def save_workflow(workflow_data: Dict = Body(...)):
    ...
    try:
        with Session(engine) as session:
            ...
            return JSONResponse(content={"status": "success", "id": workflow.id})
    except Exception as e:
...
async def list_workflows():
    ...
    try:
        with Session(engine) as session:
            ...
            return JSONResponse(content=[wf.model_dump() for wf in workflows])
    except Exception as e:
...
async def get_workflow(workflow_id: int):
    ...
    try:
        with Session(engine) as session:
            ...
            return JSONResponse(content=workflow.model_dump())
    except Exception as e:
        logger.error(f"Error loading workflow {workflow_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/workflows/{workflow_id}/execute")
async def execute_workflow_endpoint(workflow_id: int, background_tasks: BackgroundTasks):
    """Execute a workflow."""
    try:
        from ai_engine.workflow_engine import execute_workflow_by_id
        execution_id = execute_workflow_by_id(workflow_id)
        return JSONResponse(content={"status": "execution_started", "execution_id": execution_id})
    except Exception as e:
...
# --------------------------------------------------------------------------- #
# Adaptive-Learning demo endpoints (Phase-2 proof-of-concept)                 #
# --------------------------------------------------------------------------- #

@app.post("/ai/analyze_recording")
async def ai_analyze_recording(payload: Dict = Body(...)):
    """
    Mock endpoint – AI analyzes a raw recording payload and extracts
    high-level business actions.  This would normally call an internal
    learning engine, but here we return a static example so the route
    is visible in the running demo.
    """
    # In a real implementation you would feed `payload` to an AI model.
    example_actions = [
        {
            "id": "action_01",
            "title": "Receive New Load Request",
            "description": "AI detected a load request email and extracted origin / destination.",
            "type": "system"
        },
        {
            "id": "action_02",
            "title": "Negotiate Rate",
            "description": "AI observed user negotiating rate with broker in email thread.",
            "type": "communication"
        },
    ]
    return JSONResponse(content={
        ...
        "notes": "This is a mock response – replace with real AI analysis."
    })


@app.get("/ai/suggest_integrations")
async def ai_suggest_integrations():
    """
    Mock endpoint – AI suggests which software integrations provide the
    highest value based on observed user behaviour.
    """
    suggestions = [
        {"app": "Outlook", "integration_type": "oauth", "confidence": 0.92},
        {"app": "Slack", "integration_type": "api_token", "confidence": 0.74},
        {"app": "Generic UI Automation", "integration_type": "fallback", "confidence": 0.60},
    ]
    return JSONResponse(content={
        "status": "ok",
...
@app.get("/ai/learning_dashboard", response_class=JSONResponse)
async def ai_learning_dashboard():
    """
    Returns a high-level summary of what the Adaptive-Learning engine
    has learned so far – placeholder data for demo purposes.
    """
    dashboard = {
        "total_recordings_processed": 3,
        "unique_apps_detected": ["Outlook", "Chrome", "Custom TMS"],
        "confidence_score": 0.83,
        "next_best_action": "Connect Outlook via OAuth for reliable email parsing"
    }
    return dashboard
...
pattern = "**/*.py"
        ...
        if not os.path.exists("requirements.txt"):
            with open("requirements.txt", "w") as f:
                f.write("""
fastapi uvicorn sqlmodel jinja2 python-multipart requests playwright openai
                """)
